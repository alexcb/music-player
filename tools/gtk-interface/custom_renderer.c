...
static void gtk_cell_renderer_border_get_property( GObject* object,
												   guint property_id,
												   GValue* value,
												   GParamSpec* pspec )
{
	...
	case PROP_SHOW_BORDER:
		g_value_set_boolean(
			value, gtk_cell_renderer_border_get_show_border( GTK_CELL_RENDERER_BORDER( object ) ) );
		break;
	default:
...
static void gtk_cell_renderer_border_set_property( GObject* object,
												   guint property_id,
												   const GValue* value,
												   GParamSpec* pspec )
{
	switch( property_id ) {
	case PROP_SHOW_BORDER:
		gtk_cell_renderer_border_set_show_border( GTK_CELL_RENDERER_BORDER( object ),
...
static void render( GtkCellRenderer* cell,
					cairo_t* cr,
					GtkWidget* widget,
...
static void gtk_cell_renderer_border_class_init( GtkCellRendererBorderClass* klass )
{
	...
	g_object_class_install_property(
		...
		PROP_SHOW_BORDER,
		g_param_spec_int(
			"show-border", "show-border", "show-border desc", 0, 2, 0, G_PARAM_READWRITE ) );
}

static void gtk_cell_renderer_border_init( GtkCellRendererBorder* cell )
{
	cell->priv = G_TYPE_INSTANCE_GET_PRIVATE(
		cell, GTK_TYPE_CELL_RENDERER_BORDER, GtkCellRendererBorderPrivate );
...
GtkCellRenderer* gtk_cell_renderer_border_new()
{
