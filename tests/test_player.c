...
void test_feeder_item( Player* player, PlaylistItem* playlist_item )
{
	...
	for( ;; ) {
		...
		if( player->load_abort_requested || control & PLAYER_CONTROL_EXIT )
			return;
		usleep( 10 );
	}
...
unsigned int testPlayerSkip()
{
	...
	assert(
		!pthread_create( &player->audio_thread, NULL, &player_audio_thread_run, (void*)player ) );

	player->playlist_item_to_buffer = playlist->root;
	assert(
		!pthread_create( &player->reader_thread, NULL, &player_reader_thread_run, (void*)player ) );
	...
	while( wait_until_track < 10 ) {
		...
		while( last_track_seen < wait_until_track ) {
			if( !printed ) {
				printf(
					"last_track_seen=%x waiting_until=%x\n", last_track_seen, wait_until_track );
				printed = true;
			}
			usleep( 10000 );
		}
