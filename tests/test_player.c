...
unsigned int testPlayerSkip()
{
	...
	TEST_ASSERT( !pthread_create( &feeder_thread, NULL, &test_feeder, (void*) NULL ) );
	...
	for( PlaylistItem *current_item = playlist->root; current_item; current_item = current_item->next, track_num++ ) {
		LOG_DEBUG("adding pqi");
		usleep(1);
		pthread_mutex_lock( &player->the_lock );
